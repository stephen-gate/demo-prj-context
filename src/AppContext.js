...
import classes from "./AppContext.module.css";
...
const AppContext = () => {
  ...
  // prettier-ignore
  const [nextAvailableId, setNextAvailableId] = useState(INIT_APPOINTMENT_DATA.length);
  ...
  const handleClickAddNew = () => {
    if (!authorizationCtx.hasAuthorization){
      authorizationCtx.showAuthorization('ADD');
      return;
    }
  ...
  return (
    <div className={classes.div_static_plate}>
      ...
      {(authorizationCtx.warningText !=='') && <Warning /> }
